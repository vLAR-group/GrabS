...
from MinkowskiEngine import MinkowskiSigmoid
import numpy as np
from torch.nn import functional as F
from mask3d_models.modules.common import conv
...
from mask3d_models.modules.helpers_3detr import GenericMLP
...
class Mask3D(nn.Module):
    def __init__(
        ...
        self.point_sem_head = conv(self.mask_dim, self.num_classes, kernel_size=1, stride=1, bias=True, D=3)
    ...
    def forward(self, x, point2segment=None, raw_coordinates=None, train_on_segments=True, is_eval=False, anchor=None, is_datacollect=False, env_num=None):
        ...
        with torch.no_grad():
            coordinates = me.SparseTensor(features=raw_coordinates, coordinate_manager=aux[-1].coordinate_manager,
                coordinate_map_key=aux[-1].coordinate_map_key, device=aux[-1].device)
        ...
        mask_features = self.mask_features_head(pcd_features)#.detach()
        point_sem_prob = self.sigmoid(self.point_sem_head(mask_features))
        ...
        if is_datacollect:
            if env_num is not None:
                fps_idx = [furthest_point_sample(x.decomposed_coordinates[i][None, ...].float(), env_num).squeeze(0).long()
                    for i in range(len(x.decomposed_coordinates))]
                sampled_coords = torch.stack([coordinates.decomposed_features[i][fps_idx[i].long(), :] for i in range(len(fps_idx))])
            return {"sampled_coords": sampled_coords.detach() if sampled_coords is not None else None, "mask_features": mask_features}
        else:
            predictions_class = []
            predictions_mask = []
            ...
            for decoder_counter in range(self.num_decoders):
                ...
                for i, hlevel in enumerate(self.hlevels):
                    ...
                    '''new end'''
                    decomposed_aux = aux[hlevel].decomposed_features## list of bs length
                    ...
                    batched_pos_enc = torch.stack([pos_encodings_pcd[hlevel][0][k][rand_idx[k], :] for k in range(len(rand_idx))])## batched pos_enc for key and value
                    '''new'''
            ...
            return {"pred_logits": predictions_class[-1],
                ...
                "point_sem_prob": point_sem_prob.decomposed_features,# 1 is chair, 0 is bg
                "sparsed_xyz": coords[-1].decomposed_features,
                "batched_point_feats": batched_aux}


    def mask_module(self, query_feat, mask_features, mask_segments, num_pooling_steps, ret_attn_mask=True, point2segment=None, patch_mask=None, sppatch_mask=None):
