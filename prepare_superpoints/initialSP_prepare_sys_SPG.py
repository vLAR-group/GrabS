...
input_path = './data/sys_scene/processed/'
sp_save_path = './data/sys_scene/SPG_0.01/'
ignore_label = -1
voxel_size = 0.05
...
def construct_superpoints(path):
    ...
    if os.path.exists(sp_save_path + '/' + f.parts[-1][0:-4] + '_superpoint.npy'):
        ...
        coords, labels, unique_map, inverse_map = ME.utils.sparse_quantize(np.ascontiguousarray(coords), labels=instance, ignore_label=-1, return_index=True, return_inverse=True) ### labels is not used later
        ...
        features = geof#np.hstack((geof)).astype('float32')  # add rgb as a feature for partitioning
