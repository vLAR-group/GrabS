...
input_path = './data/safe_scene/processed/'
sp_save_path = './data/safe_scene/growsp_vccs5/'
ignore_label = -1
voxel_size = 0.05
...
def region_growing_simple(coords, normals=None):
    ...
    clusters = pclpy.region_growing(pc, normals=normals, min_size=1, max_size=100000, n_neighbours=15, smooth_threshold=3, curvature_threshold=1, residual_threshold=1)
...
def construct_superpoints(path):
    ...
    if os.path.exists(sp_save_path + '/' + f.parts[-1][0:-4] + '_superpoint.npy'):
        ...
        '''VCCS'''
        out = supervoxel_clustering(coords, feats, normals=normals)
        ...
        '''Region Growing'''
        clusters = region_growing_simple(coords, normals=normals)[0]#, normals)[0]
...
for split in ['train', 'test']:
    path_list += sorted(glob(os.path.join(input_path, split, '*.npz')))
