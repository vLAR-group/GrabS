...
from models import training_rectflowseg_scannet
import torch
import logging
import argparse
import os
...
def config_parser():
    ...
    parser.add_argument("--save_path", type=str, default='./segnet/scannet_rectflow_chair')
...
def main(cfg, logger):
    ...
    diffuse_net = model.Diffusion_net_sum().eval()
    ...
    #########################
    trainer = training_rectflowseg_scannet.Trainer(mask3d, objnet, actor, critic, logger, train_dataset, val_dataset, val_RL_dataset, cfg.save_path, cfg, use_norm=cfg.use_norm, use_label=False)
