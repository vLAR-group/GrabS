...
class Trainer(object):
    ...
    def validation_pseudo(self, vis=True, log=False):
        ...
        # pred_instance_color = np.vstack(get_evenly_distributed_colors(6*self.model.num_queries))
        for batch_idx, batch in enumerate(train_data_loader):
            coords, feature, normals, target, scene_name, semantic, instance, inverse_map, unique_map, voxl_pc, full_pcs, voxl_sp, pointsp, voxl_mysp, exist_pseudo = batch
            ### pseudo mask, load and save, only this phase will take 50seconds
            for b in range(len(exist_pseudo)):
                ...
                # cur_score = []
                # for idx in range(cur_pseudo.shape[1]):
                #     iou = self.get_maxmatch_mask(target[b]['masks'].t(), cur_pseudo[:, idx]).item()
                #     print(cur_pseudo.shape)
                ...
                if vis:
                    with torch.no_grad():
                        ...
                        os.makedirs(self.cfg.save_path + '/vis_pseudo', exist_ok=True)
                        ...
                        write_ply(os.path.join(self.cfg.save_path + '/vis_pseudo', scene_name[b] + 'preds.ply'), [full_pc, predcolor.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])

                        if len(target[b]['masks'])>0:
                        ...
                        write_ply(os.path.join(self.cfg.save_path + 'vis_pseudo', scene_name[b] + 'gt.ply'), [full_pc, gtcolor.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
                ...
                self.preds[scene_name[b]] = {"pred_masks": valid_masks.cpu().numpy(), "pred_scores": (torch.tensor(mask_score)).cpu().numpy(), "pred_classes": 5 * torch.ones(valid_masks.shape[-1]).cpu().numpy()}
                gt_file = os.path.join(self.cfg.data_dir, 'instance_gt', self.train_dataset.mode, scene_name[b] + '.txt')
    ...
    def validation(self, vis=True, log=False):
        ...
        for batch_idx, batch in enumerate(val_data_loader):
            ...
            self.preds[scene_name[0]] = {"pred_masks": valid_masks.cpu().numpy(), "pred_scores": (torch.tensor(mask_score)).cpu().numpy(), "pred_classes": 5 * torch.ones(valid_masks.shape[-1]).cpu().numpy()}
    ...
    def save_scannet_format(self):
        ...
        for batch_idx, batch in enumerate(val_data_loader):
            with torch.no_grad():
                ...
                os.makedirs(self.cfg.save_path + '/scannet_format', exist_ok=True)
                os.makedirs(self.cfg.save_path + '/scannet_format/predicted_masks', exist_ok=True)
                ############ save into scannet format
                f = open(os.path.join(self.cfg.save_path + '/scannet_format', f"{scene_name[0]}.txt"), "w")
                cnt = 0
                label_id = 5
                for mask_id in range(valid_masks.shape[1]):
                    single_score = float(mask_score[mask_id]) ### float
                    single_mask = valid_masks[:, mask_id].numpy().astype(np.uint8)

                    rel_fn = f"predicted_masks/{scene_name[0]}_{cnt:03d}.txt"
                    f.write(f"{rel_fn} {label_id} {single_score:.4f}\n")  ### meta file
                    mask_path = os.path.join(self.cfg.save_path + '/scannet_format', rel_fn)
            ...
            if vis:
                with torch.no_grad():
                    ...
                    for mask_id in range(valid_masks.shape[1]):
                        ...
                        predcolor[mask] = pred_instance_color[mask_id]
                        # write_ply(os.path.join(self.cfg.save_path + '/vis', scene_name[0] + 'preds_'+str(mask_id)+'.ply'), [full_pc, predcolor2.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
                    write_ply(os.path.join(self.cfg.save_path + path +scene_name[0] + '/preds.ply'), [full_pc, predcolor.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])

                    if len(target[0]['masks']) > 0:
                        gt_instance_color = np.vstack(get_evenly_distributed_colors(len(target[0]['masks'])))
                    for mask_id in range(len(target[0]['masks'])):
                        gtcolor[target[0]['masks'][:, inverse_map[0]][mask_id]] = gt_instance_color[mask_id]
                    write_ply(os.path.join(self.cfg.save_path + path + scene_name[0] + '/gt.ply'),[full_pc, gtcolor.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
                    ...
                    for traj_id in self.traj_dict.keys():
                        ...
                        write_ply(os.path.join(self.cfg.save_path + path + scene_name[0] + '/env' + traj_id + '.ply'),
                                  [full_pc, env_color.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
    ...
    def validation_dynamic_vis(self):
        self.load_checkpoint()
        self.refresh_info()
        self.model.eval(), self.actor.eval(), self.critic.eval()
        val_data_loader = self.val_RL_dataset.get_loader(shuffle=False)
        ...
        for batch_idx, batch in enumerate(val_data_loader):
            ...
            with torch.no_grad():
                ...
                valid_traj_masks, mask_score, traj_masks, traj_score = [], [], {}, {}
                traj_indictaion, traj_pos, traj_r, traj_ply = {}, {}, {}, {}

            step_R, step_num, traj_num = 0, 0, 0
            # Here we represent state by the point_idx
            state_index = []
            pc2anchor = (pc[0][:, None, :] - batch_anchor[0][None, ...])[:, :, 0:2].norm(p=2, dim=-1)  # [x, y]
            for anchor_idx in range(self.model.num_queries):
                in_env_idx = torch.where(pc2anchor[:, anchor_idx] <= self.anchor_env_r)[0]
                state_index.append(in_env_idx)

            state_index_full = []
            pc2anchor = (full_pc[0][:, None, :].cuda() - batch_anchor[0][None, ...])[:, :, 0:2].norm(p=2, dim=-1)  # [x, y]
            for anchor_idx in range(self.model.num_queries):
                state_index_full.append(torch.where(pc2anchor[:, anchor_idx] <= self.anchor_env_r)[0])

            self.init_traj_dict()
            for traj_id in range(self.model.num_queries):
                traj_masks[str(traj_id)], traj_score[str(traj_id)] = [], []
                traj_pos[str(traj_id)], traj_r[str(traj_id)] = [], []
                traj_indictaion[str(traj_id)] = []
                traj_ply[str(traj_id)] = []
                cur_env = traj_id
                all_actions, history, bcyl_center = [], torch.zeros((self.max_eval_step, 6+1)).cuda(), batch_anchor[0][cur_env].unsqueeze(0)
                ##
                curpos, initial_bcyl_center = bcyl_center.clone(), bcyl_center.clone()
            ...
            for t in range(self.max_eval_step):
                ...
                if len(not_done_traj) == 0:
                    ...
                    sdfmask, inmask_pc, inmask_norm, inmask_prob = self.compute_sdfmask_forvis(not_done_traj, batch_sp=batch_sp, state_index=[state_index])###only for compute reward, when we take the action, what reward can we have?
                    if inmask_pc is not None:
                        valid_mask, pc2mesh, mesh2pc, ply_data_list = self.compute_convergence(sdfmask, inmask_pc, inmask_prob)
                    for idx, traj_id in enumerate(not_done_traj):
                        if inmask_pc is not None:
                            reward, pc2mesh_inrange_ratio, mesh2pc_inrange_ratio, plydata = self.compute_reward_CD(idx, valid_mask, pc2mesh, mesh2pc, ply_data_list)
                        else:
                            reward = -1
                        if self.traj_dict[traj_id]['cur_GT_bcyl_mask'] is not None:
                            iou = self.get_maxmatch_mask(self.traj_dict[traj_id]['cur_GT_bcyl_mask'], self.traj_dict[traj_id]['mask_completeness'], sdfmask[idx])
                        ...
                        outmask = torch.zeros_like(pc[0])[:, 0].float()
                        outmask[state_index[self.traj_dict[traj_id]['cur_env']]] = sdfmask[idx]
                        traj_pos[traj_id].append(self.traj_dict[traj_id]['curpos'].squeeze().cpu().numpy()), traj_r[traj_id].append(self.traj_dict[traj_id]['curR'])
                        if inmask_pc is not None:
                            traj_ply[traj_id].append(plydata)

                        traj_masks[traj_id].append(outmask[inverse_map[0]].detach().cpu())
                        if inmask_prob is None:
                            traj_indictaion[traj_id].append(inmask_prob)
                        else:
                            traj_indictaion[traj_id].append(inmask_prob[idx])

                        if done:
                            if reward == self.nu and iou>=0.5:
                                traj_score[traj_id].append(1)
                            else:
                                traj_score[traj_id].append(0)
                            # ranking_score = iou#pc2mesh_inrange_ratio * torch.clamp(mesh2pc_inrange_ratio, min=0, max=1)
                            # traj_masks[traj_id].append(outmask[inverse_map[0]].unsqueeze(-1).detach().cpu()), traj_score[traj_id].append(ranking_score)

            cylinder_color = [233, 158, 143, 200]  # 233,158,143 with some transparency
            cylinder_height = full_pc[0][:, -1].max().item()*0.8

            torch.cuda.empty_cache()
            torch.cuda.synchronize(torch.device("cuda"))
            for traj_id in traj_masks.keys():
                if traj_score[traj_id][0] ==1:
                    valid_traj_masks.append(traj_masks[traj_id])
            ### vis traj
            if len(valid_traj_masks)>0:
                # pred_instance_color = np.vstack(get_evenly_distributed_colors(len(valid_traj_masks)))
                path = '/vis_valid_traj2/'
                full_pc = full_pc[0].numpy()

                for traj_id in traj_masks.keys():
                    if traj_score[traj_id][0] == 1:
                        good_traj = True
                        for step in range(len(traj_masks[traj_id])):
                            if traj_masks[traj_id][step].sum()<50:
                                good_traj = False
                                break

                        if good_traj:
                            curr_traj = traj_masks[traj_id]
                            if len(curr_traj)>=5:
                                print(scene_name[0], traj_id)
                            with torch.no_grad():
                                os.makedirs(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id)+ '/env/', exist_ok=True)
                                os.makedirs(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id)+ '/scene/', exist_ok=True)
                                os.makedirs(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id)+ '/cyl/', exist_ok=True)
                                os.makedirs(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id)+ '/obj/', exist_ok=True)


                                cyl_param = []

                                for step in range(len(curr_traj)):
                                    env_xyz = full_pc[state_index_full[int(traj_id)].cpu().long()]
                                    env_color = np.ones_like(full_pc) * 128
                                    env_color[torch.where(curr_traj[step]==1)[0]] = pred_instance_color[int(traj_id)]
                                    env_color = env_color[state_index_full[int(traj_id)].cpu().long()]
                                    write_ply(os.path.join(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id) + '/env/' +str(step) + '_env.ply'),
                                              [env_xyz, env_color.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])

                                    scene_pc = full_pc
                                    scene_color = np.ones_like(full_pc) * 128
                                    scene_color[curr_traj[step]==1] = pred_instance_color[int(traj_id)]
                                    write_ply(os.path.join(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id) + '/scene/' + str(
                                            step) + '_scene.ply'), [scene_pc, scene_color.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])

                                    cylinder_radius = traj_r[traj_id][step]
                                    cur_center = traj_pos[traj_id][step]
                                    # mesh = trimesh.load(os.path.join(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id) + '/scene/' + str(step) + '.ply'))
                                    # scene = trimesh.Scene()
                                    # scene.add_geometry(mesh)
                                    cylinder = trimesh.creation.cylinder(radius=cylinder_radius, height=cylinder_height, sections=100)
                                    cylinder.apply_translation([cur_center[0], cur_center[1], cylinder_height / 2+0.1])
                                    cylinder.visual.face_colors = np.array(cylinder_color * cylinder.vertices.shape[0]).reshape(-1, 4)
                                    cylinder.visual.vertex_colors = np.array(cylinder_color * cylinder.vertices.shape[0]).reshape(-1, 4)
                                    # scene.add_geometry(cylinder)
                                    # scene.show()
                                    cylinder.export(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id) + '/cyl/' + str(step) + '_.obj')
                                    cyl_param.append(np.concatenate((cur_center, np.array(cylinder_height)[None, ...]))[None, :]) ## 4 dimensions, x, y, _, H
                                    traj_ply[traj_id][step].write(os.path.join(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id) + '/obj/' + str(step) + '_recovered.ply'))
                                np.save(self.cfg.save_path + path + scene_name[0] + '/' + str(traj_id) + '/cyl/'+'cyl_param.npy', np.concatenate(cyl_param, axis=0))
