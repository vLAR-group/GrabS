...
import torch
from models.VN_lib.implicit_func.layers import ResnetBlockConv1d, ResnetBlockBNFC
...
class DecoderCat(nn.Module):
    ...
    def __init__(self, input_dim=3, hidden_size=128, leaky=False, out_dim=1, **kwargs):
        ...
        # self.block0 = ResnetBlockBNFC(hidden_size)
        # self.block1 = ResnetBlockBNFC(hidden_size)
        # self.block2 = ResnetBlockBNFC(hidden_size)
        # self.block3 = ResnetBlockBNFC(hidden_size)
        # self.block4 = ResnetBlockBNFC(hidden_size)
...
class DecoderCatBN(nn.Module):
    def __init__(self, input_dim=3, hidden_size=128, leaky=False, out_dim=1, **kwargs):
        super().__init__()

        self.fc_in = nn.Conv1d(input_dim, hidden_size, 1)
