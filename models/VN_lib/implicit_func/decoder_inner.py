...
from models.VN_lib.implicit_func.onet_decoder import (
...
class DecoderInnerUNet(nn.Module):
    ''' Decoder class.
    ...
    def forward(self, p, z, c=None, **kwargs):
        batch_size, T, D = p.size()
        ## p:[bs, N, 3]
        ##z:[bs, C, 3, N], p:[bs, 1, 3, N]

        net = (p * p).sum(2, keepdim=True)

        if self.z_dim != 0:
            net_z = torch.einsum('boin,bcin->bocn', p.transpose(1, -1).unsqueeze(1), z)#[bs, 1, C, N]
            z_dir = self.z_in(z)
            z_inv = (z * z_dir).sum(2)#[bs, c, N]
            net = torch.cat([net.transpose(1, -1), net_z.squeeze(1), z_inv], dim=1).transpose(1, -1)
