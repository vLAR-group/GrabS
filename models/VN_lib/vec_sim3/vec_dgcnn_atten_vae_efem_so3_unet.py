    ...
    def __init__(
        ...
        for layer_id in range(len(self.down_sample_layers)):
            if layer_id< len(self.down_sample_layers)-1:
                ...
                self.up_layers.append(VecLNA(c_dim + feat_dim[curr_layer - 1], c_dim, mode="so3", act_func=act_func))#, shared_nonlinearity=True))
            else:
                curr_layer= self.down_sample_layers[layer_id]
                self.up_layers.append(VecLNA(feat_dim[-1] + feat_dim[curr_layer - 1], c_dim, mode="so3", act_func=act_func))#, shared_nonlinearity=True))
    ...
    def forward(self, x, label=None):
        ...
        for i in range(self.num_layers):
            ...
            if i < self.atten_start_layer:
                ...
                q = self.Q_list[i](dst_f)#[B, c, 3, N]
                v = self.V_list[i](src_nn_f)# B,C,3,N,K
                k = channel_equi_vec_normalize(k)
                q = channel_equi_vec_normalize(q)
                qk = (k * q[..., None]).sum(2)# B,C,N,K  # B,C,N,K
                B, C, N, K = qk.shape
        ...
        if label is None:
            x = self.conv_c(features_on).mean(dim=-1, keepdim=False)
        else:
            label = label[:, None, None, :]#[bs, 1, 1, N]
            x = (self.conv_c(features_on) * label).sum(dim=-1, keepdim=False)/label.sum(dim=-1, keepdim=False)
        ############################################################
        ...
        ##################
        scale = (x.norm(dim=-1)+1e-12).mean(1) * self.scale_factor
