...
class VecDGCNN_att_softmask(nn.Module):
    def __init__(
        self,
        c_dim=256,
        num_layers=8,
    ...
    def get_graph_feature(
        ...
        softmask = torch.gather(softmask.unsqueeze(1).expand(-1, N_dst, -1), dim=-1, index=knn_idx)#[bs, 1024, 16]
        return y, softmask  # B,C*2,3,N,K
    ...
    def down_sample(self, x, f, factor, softmask=None):
        # x: B,1,3,N; f: B,C,3,N
        N_ori = x.shape[-1]
        N_new = N_ori // factor
        # down sample based on xyz
        ...
        f_new = torch.gather(f, dim=-1, index=idx[:, None, None, :].expand(-1, C, 3, -1))
        f_new = f_new.type(x.dtype)
        return x_new, f_new, torch.gather(softmask, dim=-1, index=idx)#[bs, N_new]


    def forward(self, x, softmask=None):
        # print(x, softmask)
        if softmask is None:
            softmask = torch.ones_like(x)[:, 0, :]

        B, _, N = x.shape

        feat_list, xyz_list = [], []
        src_softmask = softmask
        ...
        for i in range(self.num_layers):
            ...
            if i in self.down_sample_layers:
                ...
                dst_xyz, dst_f, dst_softmask = self.down_sample(src_xyz, src_f, self.down_sample_factor[self.down_sample_layers.index(i)], src_softmask)

            else:
                dst_xyz, dst_f, dst_softmask = src_xyz, src_f, src_softmask
            ...
            src_nn_f, softmask = self.get_graph_feature(src_f=src_f, dst_f=dst_f, k=num_knn, src_xyz=src_xyz, dst_xyz=dst_xyz, cross=i == 0, softmask=src_softmask)  # only has N_dst of K nn
            # * Apply MSG Passing
            if i < self.atten_start_layer:
                # message passing is through pooling
                # dst_f = self.pool(self.V_list[i](src_nn_f))
                dst_f = (self.V_list[i](src_nn_f)*softmask[:, None, None, :, :]).sum(dim=-1, keepdim=False)/(softmask[:, None, None, :, :].sum(dim=-1, keepdim=False)+1e-5)
                # sum here, because the number of neighbors are fixed
                ...
                # atten = atten*softmask[:, None, None, :, :]
                atten = torch.softmax(atten, dim=-1)
                atten = atten*softmask[:, None, None, :, :]
                atten = atten / (atten.sum(-1, keepdim=True)+1e-5)
            ...
            src_xyz, src_f, src_softmask = dst_xyz, dst_f, dst_softmask
            # if i in self.down_sample_layers or i==1:
        feat_list.append(src_f), xyz_list.append(src_xyz)

        x = self.conv_c(dst_f)#[bs, c, 3, num_points=16]
        x = x.mean(dim=-1, keepdim=False)
        # x, src_softmask = x[torch.where(src_softmask.sum(-1)>1)], src_softmask[torch.where(src_softmask.sum(-1)>1)]
        # x = (x*src_softmask[:, None, None, :]).sum(-1) / (src_softmask.sum(-1).unsqueeze(-1).unsqueeze(-1)+1e-5)
        # print(x, src_softmask.sum(-1))

        z_so3 = channel_equi_vec_normalize(x)  # without scale
        scale = x.norm(dim=-1).mean(1) * self.scale_factor
        z_inv_dual = self.fc_inv(x[..., None]).squeeze(-1)#[bs, C, 3]
        ...
        if self.center_pred:
            ...
            return center, scale, z_so3, out_inv_feat, torch.where(src_softmask.sum(-1)>-1)
        else:
            return scale, z_so3, out_inv_feat, torch.where(src_softmask.sum(-1)>-1)
