...
class VecDGCNN_att_unet(nn.Module):
    def __init__(
        ...
        self.final_feature = VecLinear(feat_dim[down_sample_layers[0]-1], 64, mode="so3")
        self.final_so3_feature = VecLinear(feat_dim[down_sample_layers[0]-1], 64, mode="so3")
    ...
    def forward(self, x):
        ...
        for i in range(-1, -(len(self.down_sample_layers)+1), -1):
            dist, idx = knn(1, xyz_list[i-1].contiguous(), xyz_list[i].contiguous())

            # dist, idx = three_nn(xyz_list[i-1].contiguous(), xyz_list[i].contiguous())
            # dist, idx = dist[:, :, :1], idx[:, :, :1]  # [B, N, 3]
            # dist = self.square_distance(xyz_list[i-1].contiguous(), xyz_list[i].contiguous())
            ...
            # f_interp_on = torch.sum(self.index_points(features_on.permute(0, 2, 1), idx) * weight.unsqueeze(-1), dim=2).permute(0, 2, 1)
            f_interp_on = self.index_points(features_on.permute(0, 2, 1), idx.long()).squeeze(2).permute(0, 2, 1)
        ...
        perpoint_feats = channel_equi_vec_normalize(perpoint_feats)
        perpoint_inv_feats = (self.final_feature(perpoint_feats) *self.final_so3_feature(perpoint_feats)).sum(2).permute(0, 2, 1) ###[bs, c, N]
