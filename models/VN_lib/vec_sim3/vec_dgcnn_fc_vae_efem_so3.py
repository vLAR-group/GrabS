    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            if use_res_global_conv and i >= self.res_global_start_layer:
                self.global_conv_list.append(VecLNA(feat_dim[i] * 2, feat_dim[i], mode="so3", act_func=act_func))
    ...
    def forward(self, x, label=None):
        ...
        for i in range(self.num_layers):
            ...
            if self.use_res_global_conv and i >= self.res_global_start_layer:
                ...
                dst_f_global = (dst_f * dst_label[:, None, None, :]).sum(dim=-1, keepdim=False) / (dst_label[:, None, None, :].sum(dim=-1, keepdim=False)+1e-12)
        ...
        x = self.conv_c(dst_f)
        y = (x * src_label[:, None, None, :]).sum(dim=-1, keepdim=False) / (src_label[:, None, None, :].sum(dim=-1, keepdim=False)+1e-12)
