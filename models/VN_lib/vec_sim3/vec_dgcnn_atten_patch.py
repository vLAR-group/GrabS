...
class VecDGCNN_att_patch(nn.Module):
    def __init__(
        self,
        c_dim=256,
        num_layers=8,
    ...
    def forward(self, x):
        ...
        x_glob = x.mean(dim=-1, keepdim=False)

        z_so3 = channel_equi_vec_normalize(x_glob)  # without scale
        scale = x_glob.norm(dim=-1).mean(1) * self.scale_factor
        # z_inv_dual = self.fc_inv(x_glob[..., None]).squeeze(-1)#[bs, C, 3]
        z_inv_dual = self.fc_inv(x).squeeze(-1)#[bs, C, 3, K]
        v_inv = (channel_equi_vec_normalize(z_inv_dual) * z_so3.unsqueeze(3)).sum(2)# [B, C, 16]
        out_inv_feat = v_inv
        out_xyz = src_xyz.squeeze(1).transpose(2, 1)### should be [B, K, 3]
        ...
        if self.center_pred:
            center = self.fc_center(x_glob[..., None]).squeeze(-1)
            if self.center_pred_scale:
                center = center * self.scale_factor
            return center, scale, z_so3, out_inv_feat, out_xyz
        else:
            return scale, z_so3, out_inv_feat, out_xyz
