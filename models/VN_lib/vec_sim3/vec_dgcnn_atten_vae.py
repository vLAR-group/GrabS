...
class VecDGCNN_att(nn.Module):
    def __init__(
        ...
        self.mean = nn.Sequential(VecLNA(feat_dim[-1], c_dim, mode="so3", act_func=act_func, shared_nonlinearity=True),
                                    VecLNA(c_dim, c_dim, mode="so3", act_func=act_func, shared_nonlinearity=True))

        self.logvar = nn.Sequential(VecLNA(feat_dim[-1], c_dim, mode="so3", act_func=act_func, shared_nonlinearity=True),
                                    VecLNA(c_dim, c_dim, mode="so3", act_func=act_func, shared_nonlinearity=True))
    ...
    def forward(self, x):
        ...
        z_so3_mean = channel_equi_vec_normalize(mean)  # without scale
        z_inv_mean = self.fc_inv_mean(mean[..., None]).squeeze(-1)#[bs, C, 3]
        inv_mean = (channel_equi_vec_normalize(z_inv_mean) * z_so3_mean).sum(-1)

        z_so3_logvar = channel_equi_vec_normalize(logvar)  # without scale
        z_inv_logvar = self.fc_inv_logvar(logvar[..., None]).squeeze(-1)#[bs, C, 3]
        inv_logvar = (channel_equi_vec_normalize(z_inv_logvar) * z_so3_logvar).sum(-1)
