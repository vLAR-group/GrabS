...
class VoxelizedDataset(Dataset):
    def __init__(self, mode, cfg, data_path , batch_size, num_workers, voxel_size, RL=False):
        ...
        for scene_name in scene_list:
            ...
            if self.mode == 'train':
                # if ("0636_00" not in scene_name) and ("0154_00" not in scene_name) and (semantic==5).sum()>0:
    ...
    def __getitem__(self, idx):
        ...
        # pc, color, normals, sp_idx = points[:, :3], points[:, 3:6], points[:, 6:9], points[:, 9]
        # semantic, instance = np.zeros_like(sp_idx), np.zeros_like(sp_idx)
        # pc = pc - pc.mean(0)
        pc = pc - pc.min(0)
        ...
        #     with open(efemgt_file, 'r') as f:
        #         data = f.readlines()
        #     # pseudo = np.zeros_like(instance).repeat(len(data), 1)
        #     # pseudo_score = []
        ...
        #         # pseudo[:, mask_id] = mask
        #         # pseudo_score.append(float(data[mask_id].split(' ')[-1][:-2]))
        ...
        # coords, feature, _, _, unique_map, inverse_map = self.voxelize(pc, feature, semantic, instance)
        ...
        if self.cfg.sp_dir is not None:
            mysp_name = os.path.join(self.cfg.sp_dir, scene_name + '_superpoint.npy')
            mysp = np.load(mysp_name)
            if len(mysp.shape) == 2:
                mysp = mysp.squeeze(1)
            mysp_voxel = mysp[unique_map]
        else:
            mysp_voxel = sp_idx[unique_map]
        ####
        ...
        if os.path.exists(exist_mask_file):
            ...
            exist_mask = [torch.zeros((len(semantic), 1)).bool(), torch.tensor(0).unsqueeze(-1)]
        return coords, feature, normals, semantic.squeeze(), instance.squeeze(), inverse_map, unique_map, scene_name, pc, pc_full, torch.from_numpy(sp_idx_voxel).long(), torch.from_numpy(sp_idx).long(), torch.from_numpy(mysp_voxel).long(), exist_mask
    ...
    def collate_fn(self, batch):
        coords, feature, normals, semantic, instance, inverse_map, unique_map, scene_name, pc, pc_full, sp_idx, sp_idx_full, mysp, exist_mask = list(zip(*batch))
        ...
        for batch_id, _ in enumerate(coords):
            ...
            if len(masks)>0:
                ...
                # self.above_num += (torch.cat(masks, dim=0).squeeze(-1).sum(-1)>200).sum()
        ...
        return coords_batch, feature_batch, normals_batch, target, scene_name, semantic_batch, instance_batch, inverse_map, unique_map, pc_batch, pc_batch_full, sp_batch, sp_batch_full, mysp_batch, exist_mask_batch