...
# classes = ['02691156', '04401088', '02933112', '04379243', '04530566', '04090263', '03001627', '04256520']
classes = ['02691156', '04401088', '02933112', '04090263', '03001627', '04256520']

class VoxelizedDataset(Dataset):
    def __init__(self, mode, data_path, split_file_folder , batch_size, num_sample_points,
                 num_workers, mix_file_dir, mix_splits):
        self.path = data_path
        self.mode = mode
        self.data = []
        self.count = []
        ...
        for cls in classes:
            train_split_file = os.path.join(split_file_folder, cls, '%s.lst' % (self.mode))
            with open(train_split_file, 'r') as f:
                train_model_files = f.read().split('\n')
                train_model_files = list(filter(lambda x: len(x) > 0, train_model_files))
            for item in train_model_files:
                self.data.append(os.path.join(cls, item))
            self.count.append(len(train_model_files))
        ...
        if mix_file_dir is not None:
            ...
            for mix_class in mix_classes:
                if mix_class in ['03001627', '02933112', '04256520', '04379243', '02828884', '03211117']:
    ...
    def __getitem__(self, idx):
        ...
        cls = classes.index(path.split('/')[-2])
        ...
        if self.mode == 'val':
            ...
            on_surface_points, off_surface_points, df = on_surface_points / scale, off_surface_points / scale, df / scale
            ...
            on_surface_points, off_surface_points = on_surface_points[:, [0, 2, 1]], off_surface_points[:, [0, 2, 1]]

            on_surface_points = np.dot(on_surface_points, random_R2)
            off_surface_points = np.dot(off_surface_points, random_R2)

            return {'on_surface_points': on_surface_points, 'df': df, 'off_surface_points': off_surface_points, 'path': path, 'cls': np.array(int(cls)),
