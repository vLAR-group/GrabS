...
class Trainer(object):
        ...
        for iter in range(iters+1):
            ...
            with torch.no_grad():
                ...
                while head < len(valid_traj_id_tmp):
                    pred_R_inv = self.objnet.VAE.get_pred_R_inv(input_pc[head:min(head + self.sdf_bs, len(valid_traj_id_tmp))])
                    cond_feat = self.objnet.cond_net(torch.bmm(input_pc[head:min(head + self.sdf_bs, len(valid_traj_id_tmp))], pred_R_inv).detach())
                    embedd = [self.generation(cond_feat), pred_R_inv]
            ...
            for id, traj_id in enumerate(valid_traj_id_tmp):
                ...
                if self.use_norm:
                    ...
                    cur_onsurf_sdf = self.objnet.VAE.decode(query_points, *tmp_embedding).abs()  # [bs*5, N]
        ...
        if np.array(valid_mask).sum()>0:
            ...
            with torch.no_grad():
                while head < encoder_input_pc.shape[0]:
                    pred_R_inv = self.objnet.VAE.get_pred_R_inv(encoder_input_pc[head:min(head + self.sdf_bs, encoder_input_pc.shape[0])])
                    cond_feat = self.objnet.cond_net(torch.bmm(encoder_input_pc[head:min(head + self.sdf_bs, encoder_input_pc.shape[0])], pred_R_inv).detach())
                    embedd = [self.generation(cond_feat), pred_R_inv]

                    canonical_query_pc = query_pc[head:min(head + self.sdf_bs, encoder_input_pc.shape[0])]# / embedd['s'][:, None, None]
                    mesh_pts, validness = self.extract_shape_pts(self.objnet.VAE.decode, embedd, sample_pts_num=query_pc.shape[1], N=32)
    ...
    def compute_reward_CD(self, idx, valid_mask, pc2meshs, mesh2pcs):
        if valid_mask[idx]:# and (valid_mask[idx].sum()>=self.mask_min_size):
            true_list = [i for i, x in enumerate(valid_mask) if x]
            idx_in_true_list = true_list.index(idx)
            pc2mesh = pc2meshs[idx_in_true_list]
            mesh2pc = mesh2pcs[idx_in_true_list]
        ...
        for batch_idx, batch in enumerate(val_data_loader):
            ...
            for t in range(self.max_eval_step):
                ...
                if len(not_done_traj) == 0:
                    ...
                    for idx, traj_id in enumerate(not_done_traj):
                        if inmask_pc is not None:
                            reward, pc2mesh_inrange_ratio, mesh2pc_inrange_ratio = self.compute_reward(idx, valid_mask, pc2mesh, mesh2pc)
                        else:
                            reward = -1
                        if self.traj_dict[traj_id]['cur_GT_bcyl_mask'] is not None:
    ...
    def generation(self, cond):
        with torch.no_grad():
            ...
            for j in range(self.sample_steps):
                ...
                if cond is not None:
                    v_pred_uncond = self.objnet.diffusion_net(x_t, torch.zeros_like(cond), t=t*1000)
                    v_pred_cond = self.objnet.diffusion_net(x_t, cond, t=t*1000)
                    v_pred = v_pred_uncond + self.cond_strength * (v_pred_cond - v_pred_uncond)
                else:
                    v_pred = self.objnet.diffusion_net(x_t, cond, t=t*1000)
                ## euler
                x_t = x_t + v_pred * dt
        return x_t
